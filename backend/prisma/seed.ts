...
import { INITIAL_PRESCRIPTIONS, PATIENT_DATA, DOCTORS, VITALS_DATA, DOCUMENTS } from '../src/data/mockData.js';
...
async function main() {
  ...
  for (const rx of INITIAL_PRESCRIPTIONS) {
    ...
    await prisma.prescription.upsert({
      ...
      create: {
        id: rx.id,
        patientId: 'patient-robert-c',
        doctorId: doctor?.id,
        name: rx.name,
        dosage: rx.dosage,
        manufacturer: rx.manufacturer,
        brandEquivalent: rx.brandEquivalent,
        status: rx.status,
        statusLabel: rx.statusLabel,
        remainingInfo: rx.remainingInfo,
        daysLeft: rx.daysLeft,
        instructions: rx.instructions,
        directGenericPrice: rx.directGenericPrice,
        innovatorPrice: rx.innovatorPrice,
      },
    });
  }

  for (const vital of VITALS_DATA) {
    await prisma.vitalMetric.create({
      data: {
        patientId: 'patient-robert-c',
        title: vital.title,
